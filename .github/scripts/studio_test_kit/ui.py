    ...
    try:
        async with async_playwright() as p:
            ...
            if video_dir is not None:
                ...
                # Snapshot files that already exist; Playwright versions differ
                # between page@<hash>.webm and <hash>.webm naming.
                pre_existing = set(video_dir.glob("*.webm"))
        ...
        if sp is not None and video_dir is not None:
            try:
                ...
                new_webms = sorted(
                    p for p in video_dir.glob("*.webm")
